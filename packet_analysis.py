...
def packet_handler(packet_to_process, printing_queue):
    # print(packet_to_process)
    if packet_to_process.haslayer(HTTPRequest):
        process_http_packet(packet_to_process, printing_queue)


def sniffer_loop_scapy(interface_to_capture_packets, printing_queue, verbosity, cancel_token):
    printing_queue.put(f"{GREEN}[+] Starting sniffer{RESET}")
    while not cancel_token.is_set():
        sniff(iface=interface_to_capture_packets,
              prn=lambda packet_to_process: packet_handler(packet_to_process, printing_queue), store=0, count=1)
...
def start_sniffer_thread(interface_to_capture_packets, printing_queue, verbosity=0):
    ...
    sniffer_thread = threading.Thread(target=sniffer_loop_scapy,
                                      args=(interface_to_capture_packets, printing_queue, verbosity, cancel_token))
